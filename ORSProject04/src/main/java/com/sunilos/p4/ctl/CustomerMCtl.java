...
@WebServlet("/ctl/CustomerMCtl")
public class CustomerMCtl extends BaseCtl<CustomerMBean, CustomerMModel> {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(CustomerMCtl.class);
	...
	protected String getView() {
		return ORSView.CUSTOMER_M_VIEW;
	}

	@Override
	protected String getView(String op) {
		if (OP_CANCEL.equalsIgnoreCase(op) || OP_DELETE.equalsIgnoreCase(op)) {
			return ORSView.CUSTOMER_M_LIST_CTL;
		} else {
			return ORSView.CUSTOMER_M_VIEW;
		}
	}
